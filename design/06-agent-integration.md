...
| File change diffs  | No stable typed SDK diff contract; derive best-effort diffs from observed tool results when available | turn/diff/updated, fileChange items with diffs        | Codex native turn diff is authoritative; Claude tool-result diff extraction is heuristic          |
