...
- `/apps/desktop`: Electron shell. Discovers or launches the Forge daemon and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@forgetools/shared/git`, `@forgetools/shared/DrainableWorker`) — no barrel index.
