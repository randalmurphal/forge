...
- Dev commands default `FORGE_HOME` to `~/.forge`.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --base-dir ~/.forge-alt`
...
- Desktop production windows load the bundled UI from `forge://app/index.html` (not a `127.0.0.1` document URL).
- Desktop packaging includes `apps/server/dist` (the Forge backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
Set `FORGE_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `FORGE_DEV_INSTANCE`)
- Example: `FORGE_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `FORGE_PORT_OFFSET` to a numeric offset.
