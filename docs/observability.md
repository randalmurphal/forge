...
Forge has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.forge/userdata/logs/server.trace.ndjson`).
...
```bash
export FORGE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export FORGE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export FORGE_OTLP_SERVICE_NAME=forge-local
```

Optional:

```bash
export FORGE_TRACE_MIN_LEVEL=Info
export FORGE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `FORGE_OTLP_*`.

macOS app bundle example:

```bash
FORGE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
FORGE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
FORGE_OTLP_SERVICE_NAME=forge-desktop \
...
```bash
tail -f "$FORGE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$FORGE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$FORGE_HOME/userdata/logs/server.trace.ndjson"
```
...
- service name such as `forge-local`, `forge-dev`, or `forge-desktop`
...
- `forge_rpc_request_duration`
- `forge_orchestration_command_duration`
- `forge_orchestration_command_ack_duration`
- `forge_provider_turn_duration`
- `forge_git_command_duration`
- `forge_db_query_duration`

Counters tell you volume and failure rate:

- `forge_rpc_requests_total`
- `forge_orchestration_commands_total`
- `forge_provider_turns_total`
- `forge_git_commands_total`
- `forge_db_queries_total`
...
`forge_orchestration_command_ack_duration` measures:
...
1. Check `forge_orchestration_command_ack_duration` by `commandType`.
...
- `FORGE_OTLP_TRACES_URL` was not set
...
- `FORGE_TRACE_FILE`: override trace file path
- `FORGE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `FORGE_TRACE_MAX_FILES`: rotated file count, default `10`
- `FORGE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `FORGE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `FORGE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `FORGE_OTLP_TRACES_URL`: OTLP trace endpoint
- `FORGE_OTLP_METRICS_URL`: OTLP metric endpoint
- `FORGE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `FORGE_OTLP_SERVICE_NAME`: service name, default `forge-server`
