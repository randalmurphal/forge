...
- Publishes the CLI package (`apps/server`, npm package `@forgetools/server`) with OIDC trusted publishing.
...
- Repository slug source:
  - `FORGE_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `FORGE_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
1. Confirm the npm org/user owns package `@forgetools/server`.
