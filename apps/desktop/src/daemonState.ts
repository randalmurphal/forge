...
export const readDaemonInfoSync = (daemonInfoPath: string): DesktopDaemonInfo | undefined => {
  try {
...
export const createDesktopWsUrlResolver = (input?: {
  ...
  const initialDaemonInfo = readSync(paths.daemonInfoPath);
  ...
  const getWsUrl = (): string | null => {
    ...
    const daemonInfo = readSync(paths.daemonInfoPath);
  ...
  const prime = (): Promise<string | null> => {
    ...
    primePromise = (async () => {
      ...
      while (Date.now() < deadline) {
        const daemonInfo = await readAsync(paths.daemonInfoPath);
        if (daemonInfo !== undefined) {
          cachedWsUrl = buildDaemonWsUrl(daemonInfo);
          return cachedWsUrl;
        }
