...
export type ComposerSlashCommand = "model" | "plan" | "default" | "design";
...
const SLASH_COMMANDS: readonly ComposerSlashCommand[] = ["model", "plan", "default", "design"];
...
export function parseStandaloneComposerSlashCommand(
  ...
  const match = /^\/(plan|default|design)\s*$/i.exec(text.trim());
  if (!match) {
    return null;
  }
  const command = match[1]?.toLowerCase();
  if (command === "plan") return "plan";
  if (command === "design") return "design";
  return "default";
}
