  ...
  const api: NativeApi = {
    ...
    shell: {
      openInEditor: async (cwd, editor) => {
        if (window.desktopBridge) {
          const handled = await window.desktopBridge.openInEditor(cwd, editor);
          if (handled) return;
        }
        return rpcClient.shell.openInEditor({ cwd, editor });
      },
    ...
    orchestration: {
      ...
      getClientSnapshot: rpcClient.orchestration.getClientSnapshot,
      getThreadDetail: rpcClient.orchestration.getThreadDetail,
      dispatchCommand: rpcClient.orchestration.dispatchCommand,
      getTurnDiff: rpcClient.orchestration.getTurnDiff,
      getFullThreadDiff: rpcClient.orchestration.getFullThreadDiff,
      getCommandOutput: rpcClient.orchestration.getCommandOutput,
      getSubagentActivityFeed: rpcClient.orchestration.getSubagentActivityFeed,
