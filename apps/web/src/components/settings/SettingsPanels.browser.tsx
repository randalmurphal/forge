...
import { DEFAULT_SERVER_SETTINGS, type NativeApi, type ServerConfig } from "@forgetools/contracts";
...
describe("GeneralSettingsPanel observability", () => {
  ...
  it("shows diagnostics inside About with a single logs-folder action", async () => {
    ...
    await expect
      .element(page.getByText("/repo/project/.forge/logs", { exact: true }))
      .toBeInTheDocument();
    await expect
      .element(
        page.getByText(
  ...
  it("opens the logs folder in the preferred editor", async () => {
    ...
    expect(openInEditor).toHaveBeenCalledWith("/repo/project/.forge/logs", "cursor");
  });
});
