...
export function PhaseCard(props: {
  ...
  const executionKind = resolveWorkflowExecutionKind(props.phase.type);
  const previousPhaseOptions = props.previousPhaseOptions;
      ...
      <div className="space-y-5 px-4 py-4 sm:px-5">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) => {
                ...
                if (value === "go-back-to" && previousPhaseOptions.length === 0) {
                  return;
                }
                props.onChange({
                  ...props.phase,
                  gate: {
                    ...
                    retryPhase:
                      value === "go-back-to"
                        ? (props.phase.gate.retryPhase ?? previousPhaseOptions[0])
                        : undefined,
                  },
                });
              }}
              ...
              <SelectPopup>
                {WORKFLOW_GATE_ON_FAIL_OPTIONS.map((option) => (
                  <SelectItem
                    ...
                    value={option.value}
                    className={
                      option.value === "go-back-to" && previousPhaseOptions.length === 0
        ...
        {props.phase.gate.onFail === "go-back-to" ? (
          <div className="space-y-2">
            ...
            <Select
              value={props.phase.gate.retryPhase ?? previousPhaseOptions[0] ?? ""}
              onValueChange={(value) => {
                if (!value) {
                  return;
                }
              ...
              disabled={props.disabled || previousPhaseOptions.length === 0}
            >
              <SelectTrigger>
                <SelectValue
                  placeholder={
                    previousPhaseOptions.length === 0
              ...
              <SelectPopup>
                {previousPhaseOptions.map((phaseName) => (
