...
export function runtimeEventToActivities(
  ...
  switch (event.type) {
    ...
    case "mcp.status.updated": {
      const summary =
        event.payload.status === "failed"
          ? `MCP server ${event.payload.name} failed to start`
          : event.payload.status === "cancelled"
            ? `MCP server ${event.payload.name} startup cancelled`
            : event.payload.status === "ready"
              ? `MCP server ${event.payload.name} started`
              : `MCP server ${event.payload.name} is starting`;
      return [
        {
          id: event.eventId,
          createdAt: event.createdAt,
