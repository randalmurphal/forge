...
import {
  FORGE_WORKTREE_BRANCH_PREFIX,
  resolveAutoFeatureBranchName,
  sanitizeBranchFragment,
  sanitizeFeatureBranchName,
} from "@forgetools/shared/git";

import { GitManagerError } from "@forgetools/contracts";
import {
  GitManager,
...
import type { GitManagerServiceError } from "@forgetools/contracts";
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `${FORGE_WORKTREE_BRANCH_PREFIX}/pr-${pullRequest.number}/${suffix}`;
}
...
export const makeGitManager = Effect.fn("makeGitManager")(function* () {
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const bodyFile = path.join(tempDir, `forge-pr-body-${process.pid}-${randomUUID()}.md`);
    yield* fileSystem
